...
public class EventBridge implements AdvancedChatEventVisitor<MessageFromTwitch,Nil> {
    ...
    public @NonNull Nil visit(@NonNull Connection<MessageFromTwitch> event) {
        return handle(event, (s,e) -> new perobobbot.plugin.twitch.chat.event.Connection(s));
    }

    @Override
    public @NonNull Nil visit(@NonNull Disconnection<MessageFromTwitch> event) {
        return handle(event, (s,e) -> new perobobbot.plugin.twitch.chat.event.Disconnection(s));
    }
    ...
    public @NonNull Nil visit(@NonNull PostedMessage<MessageFromTwitch> event) {
        return handle(event, (s,e) -> {
            ...
            return message.map(m -> new perobobbot.plugin.twitch.chat.event.PostedMessage(s, event.getDispatchingTime(), m))
                    .orElse(null);
        });
    }
    ...
    public @NonNull Nil visit(@NonNull ReceivedMessage<MessageFromTwitch> event) {
        return handle(event, (s,e) -> new perobobbot.plugin.twitch.chat.event.ReceivedMessage<>(s, e.getReceptionTime(), e.getMessage()));
    }

    @Override
    public @NonNull Nil visit(@NonNull Error<MessageFromTwitch> event) {
        return handle(event,(s,e) -> new perobobbot.plugin.twitch.chat.event.Error(s, e.getError()));
    }
